...
def objective(trial):
    ...
    # args.sub_max = True
    # args.cl_temp = 0.07
    args.dweight_loss = trial.suggest_categorical("dweight_loss", [True, False])
    args.rep_aug = trial.suggest_categorical("rep_aug", ["mean", "relative", "none"])
    args.distill = trial.suggest_categorical("distill", ["fd", "pd", "mul", "none"])
    args.cl_aug = trial.suggest_categorical("cl_aug", ["dropout", "shuffle", "RTR", "none"])
    args.joint_da_loss = trial.suggest_categorical("joint_da_loss", ["none", "ce", "dist", "mul"])
    args.aug_repeat_times = trial.suggest_int("aug_repeat_times", 2, 5)
    args.sub_max = True
    args.cl_temp = trial.suggest_float("cl_temp", 0.05, 0.5, step=0.01)
    ####
    args.tlcl = False
    args.ucl = True
    ...
    args.project_name = "HANet_ace_2_5_full"
