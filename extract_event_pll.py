    ...
    for dataset in datasets:
        ...
        for i in range(1, 2):
            ...
            if not os.path.exists(input_folder):
                loguru.logger.error(f"[ERROR] Folder {input_folder} is not exist", mode="ERROR")
                continue
            ...
            for file_name in os.listdir(input_folder):
                ...
                # Start consuming
                try:
                    results, processed_item, remained_item = consumer.consume() # results is a list of tuples (line_idx, key, idx, item)
                except KeyboardInterrupt:
                    loguru.logger.error(f"[ERROR] KeyboardInterrupt, stop consuming safely...")
        ...
        # Start consuming
        try:
            results, processed_item, remained_item = consumer.consume()
        except KeyboardInterrupt:
            loguru.logger.error(f"[ERROR] KeyboardInterrupt, stop consuming safely...")
            consumer.stop_threads()
...
if __name__ == "__main__":
    ...
    # Configure logging
    loguru.logger.add(os.path.join(logs_dir, "extractor.log"), rotation="1 MB", retention="10 days", level="INFO")
